...
from custom_components.ai_agent_ha.rag.event_handlers import (
...
class TestEntityRegistryEventHandler:
    ...
    async def test_handle_entity_registry_updated_create(self, mock_hass, mock_indexer):
        ...
        with patch.object(handler, "_on_entity_added", new_callable=AsyncMock) as mock_added:
    ...
    async def test_handle_entity_registry_updated_remove(self, mock_hass, mock_indexer):
        ...
        with patch.object(handler, "_on_entity_removed", new_callable=AsyncMock) as mock_removed:
            await handler._handle_entity_registry_updated(event)
    ...
    async def test_handle_entity_registry_updated_update(self, mock_hass, mock_indexer):
        ...
        with patch.object(handler, "_on_entity_updated", new_callable=AsyncMock) as mock_updated:
            await handler._handle_entity_registry_updated(event)
            mock_updated.assert_called_once_with("light.updated", changes)

    @pytest.mark.asyncio
    async def test_handle_entity_registry_updated_no_entity_id(self, mock_hass, mock_indexer):
...
class TestStateChangeHandler:
    ...
    @pytest.mark.asyncio
    async def test_handle_state_changed_friendly_name_changed(self, mock_hass, mock_indexer):
        """Test handling state change where friendly_name changed."""
        handler = StateChangeHandler(mock_hass, mock_indexer)
    ...
    @pytest.mark.asyncio
    async def test_handle_state_changed_friendly_name_unchanged(self, mock_hass, mock_indexer):
