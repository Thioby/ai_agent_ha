...
from custom_components.ai_agent_ha.const import DOMAIN, CONF_RAG_ENABLED
...
class TestAsyncSetupEntry:
    ...
    async def test_setup_entry_openai_provider(self, mock_hass, mock_entry):
        ...
        with patch("custom_components.ai_agent_ha.AiAgentHaAgent") as mock_agent:
    ...
    async def test_setup_entry_with_rag_enabled(self, mock_hass, mock_entry):
        ...
        with patch("custom_components.ai_agent_ha.AiAgentHaAgent") as mock_agent:
            with patch("custom_components.ai_agent_ha._initialize_rag", new_callable=AsyncMock) as mock_rag:
                result = await async_setup_entry(mock_hass, mock_entry)
    ...
    async def test_setup_entry_incompatible_version(self, mock_hass):
        ...
        with patch("custom_components.ai_agent_ha.AiAgentHaAgent") as mock_agent:
            with patch("custom_components.ai_agent_ha._LOGGER") as mock_logger:
                result = await async_setup_entry(mock_hass, mock_entry)
...
class TestAsyncUnloadEntry:
    ...
    async def test_unload_entry_success(self, mock_hass_with_agent):
        ...
        with patch("custom_components.ai_agent_ha._shutdown_rag", new_callable=AsyncMock):
            with patch("custom_components.ai_agent_ha._panel_exists", new_callable=AsyncMock, return_value=False):
...
class TestServiceHandlers:
    ...
    async def test_service_query_calls_agent(self, mock_hass_with_agent):
        ...
        with patch("custom_components.ai_agent_ha.AiAgentHaAgent", return_value=mock_agent):
            mock_entry = MagicMock(spec=ConfigEntry)
            mock_entry.version = 1
...
class TestRAGHelpers:
    ...
    async def test_initialize_rag_success(self):
        ...
        from custom_components.ai_agent_ha import _initialize_rag
        ...
        with patch("custom_components.ai_agent_ha.rag.RAGManager", return_value=mock_rag_manager):
    ...
    async def test_initialize_rag_import_error(self):
        ...
        import sys
        with patch.dict(sys.modules, {"custom_components.ai_agent_ha.rag": None}):
            with patch("custom_components.ai_agent_ha._LOGGER") as mock_logger:
    ...
    async def test_shutdown_rag_success(self):
        ...
        from custom_components.ai_agent_ha import _shutdown_rag
...
class TestPanelHelpers:
    ...
    async def test_panel_exists_true(self):
        ...
        from custom_components.ai_agent_ha import _panel_exists
    ...
    async def test_unload_entry_with_panel(self):
        ...
        with patch("custom_components.ai_agent_ha._shutdown_rag", new_callable=AsyncMock):
            with patch("custom_components.ai_agent_ha._panel_exists", new_callable=AsyncMock, return_value=True):
                with patch("homeassistant.components.frontend.async_remove_panel", mock_async_remove_panel):
                    result = await async_unload_entry(mock_hass, mock_entry)

                    assert result is True
                    mock_async_remove_panel.assert_called_once_with(mock_hass, "ai_agent_ha")
