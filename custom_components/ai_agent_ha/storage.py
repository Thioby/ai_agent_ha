"""Session storage for AI Agent HA chat conversations.
...
STORAGE_VERSION = 1
STORAGE_KEY = "ai_agent_ha_user_data"
...
class Message:
    ...
    def __post_init__(self) -> None:
        ...
        # Validate status
        if self.status not in ("pending", "completed", "error"):
...
class SessionStorage:
    ...
    async def _migrate_legacy_data(self) -> None:
        ...
        Converts old ai_agent_ha_history_{user_id} format to new session format.
        ...
        legacy_store: Store[dict[str, Any]] = Store(
            self.hass, 1, f"ai_agent_ha_history_{self.user_id}"
        )
